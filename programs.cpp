...
void update_state(int input) {
  ...
  unsigned long time_init, time_on;
  ...
  switch (plug0_state) {
    ...
    case ON:
      ...
      if (input == -1) {
        ...
        time_on = millis() - time_init;
        convert_to_hour_minutes_seconds(time_on, &hours, &minutes, &seconds);
        printf("plug was on during %d hours, %d minutes, %d seconds\n", hours, minutes, seconds);
      }
      if (input == -2) {
        plug0_state = OFF;
        plug0_program = program0;
...
void convert_to_hour_minutes_seconds(unsigned long time, int *hours, int *minutes, int *seconds){
  *hours = time / 3600000;
  time %= 3600000;
  *minutes = time / 60000;
  time %= 60000;
